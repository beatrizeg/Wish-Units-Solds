...
```
Black and white are the most common colors along units sold.
...
XS is the most common size within units sold.
...
```
Prices within 5 to 10 EUR are the most common, and that high prices are not common, reason for the smooth function to cover a big area.
...
```
We can see that it is more common to not use ad_boosts.
...
So best *span* in this case is 0.6, and we only get an accuracy of 0.05139. If we tried guessing, the probability of being correct would be 1/9 = 0.11. So we would do better just by guessing! In fact, if we use the predict function, we see that it is just predicting the same category every time.
...
The accuracy we get with this model in the train set is 0.688, higher than in the previous tree ones, so this is the one we will use for predictions.
...
*By Venkata Jagannath - : ![](Random_forest_diagram_complete.png)
...
We will start to run a model with the default values from *caret* package.
...
We see that the best accuracy we get is for a mtry=33 . As we said before, in the case best mtry>number of predictors in our dataset. This is because *caret* *train* function converts the database into a matrix, and considers, as an independent predictor, each level of the categorical variables.
...
As we have 28 predictors in our database, we will try *mtry* that range from 16 to 40. As in Random Forest we already use independent trees in the model, we will use simple cross-validation in this case.
...
And now we get a higher accuracy of 0.7082 for a mtry=30. Once that we have optimized the *mtry* parameter, we will try with different nodesizes and see if we can get any improvement.
...
Great! So the optimal nodesize=3. Now, keeping our optimal nodesize=3 and mtry=30, we will train our final model to be tested on test_set and check the results. Also, we can plot the variable importance of the model.
...
Looks like although we were able to improve the results in the train set, actually our accuracy in the test set is the same as with the default values. In this case, the optimal values of mtry and nodesize did not really have an effect. Anyways, with Random Forest we are able now to predict correctly over three quarters of the units sold.
...
We will start by optimizing the tuneGrid hyperparameters of *eta* and *maxdepth*. To start the first model, we will keep the optimal *nodesize*=3 from Random Forest.

We will vary *eta* from 0.01 to 0.3, which controls the learning rate. And *max_depth* from 4 to 10, which controls the maximum depth of the trees. If we make *max_depth* too large, we have higher chances of overfitting the model.
...
Once we have our optimized values of *eta*=0.01 and *max_depth*=8, giving an accuracy of 0.695, we will fix those values and try to optimize the *nrounds* hyperparameter. *nrounds* controls the maximum number of iterations.
...
We need to keep in mind that the higher the *nrounds* number, longer the time the model needs to run. Now that we have also optimized the *nrounds* parameter to equal 500, which provides an accuracy of 0.692, we will try to improve the model by looking at different *min_child_weight* values. It means the same as *nodesize* in Random Forest.
...
Great, we now have a new parameter optimized. With *min_child_weight*=1 we get an accuracy of 0.699. Lastly, we will see if trying a different *gamma* can improve our accuracy. *Gamma* is the regularization parameter that prevents overfitting. It defaults to 0.
...
We can see that we get the higher accuracy of 0.701 for a gamma=2. Once that we have optimized all these parameters, we train the final model.

By using the varImp function, we can plot as well the importance that the different predictors have in the model.
...
With our final XGBoost model we were not able to outperform the Random Forest model.
...
In the cross validation of the training data, we can see that it achieves an accuracy of 0.697. Also, we can study the variable importance of the model, and check if it matches the variable importance of the XGBoost model we performed.

```{r varimp h2o}
plot <- h2o.varimp_plot(automl_all@leader, num_of_features = 10)
```

Now, running the model in our test_set, we get an accuracy of (203-53)/203=0.7389.
...
acc_results
```
Personally, I think that it is always nice to check that the autoML algorithm does not beat the work done by the data scientist, and that other methods get a higher accuracy.

# 4. CONCLUSION

For this multinomial problem, we have just checked how some methods work better than others. In this case, winners are Random Forest and XGBoost, as they get very similar results. If we were having a larger dataset, time would be an important factor when deciding for which method to use, and the fastest method would get chosen.
